...
cleanup() {
  "${PRIV_CMD[@]}" ip link set dev "${RX_NIC}" xdp off >/dev/null 2>&1 || true
  "${PRIV_CMD[@]}" ip addr del 192.168.1.102/24 dev "${RX_NIC}" >/dev/null 2>&1 || true
}

trap cleanup EXIT

mkdir -p "${ROOT_DIR}/${OUTPUT_DIR}"
"${PRIV_CMD[@]}" ip addr add 192.168.1.102/24 dev "${RX_NIC}" >/dev/null 2>&1 || true

bash "${ROOT_DIR}/scripts/compile_min_xdp.sh" >/dev/null
bash "${ROOT_DIR}/scripts/build_af_xdp_rx_poc.sh" >/dev/null

"${PRIV_CMD[@]}" "${ROOT_DIR}/build_af_xdp_probe/af_xdp_rx_poc" "${RX_NIC}" "${QUEUE_ID}" "${DURATION}" "${ROOT_DIR}/build_af_xdp_probe/xdp_redirect_kern.bpf.o" \
  > "${ROOT_DIR}/${OUTPUT_DIR}/rxpoc.log" 2>&1 &
RX_PID=$!

sleep 1

"${PRIV_CMD[@]}" python3 "${ROOT_DIR}/tools/raw_eth_sender.py" \
  --iface "${TX_IFACE}" \
  --dst-mac "${RX_MAC}" \
  --src-mac "${TX_MAC}" \
  --duration "${DURATION}" \
