...
| 网络调优 | X710 `RX/TX ring = 4096`，`irqbalance = disabled`，`receiver0 MTU = 9000` |
...
- **巨型帧 (Jumbo Frame)**：
    - `receiver0` 已通过 NetworkManager 连接 `receiver0` 持久化设置 `MTU = 9000`。
    - 当前现场以 `receiver0` 作为接收验证口，巨型帧已生效。
- **硬件加速特性 (Offloading)**：
...
|---|---|---|---|---|---|---|
|`receiver0`|`enP1s25f0`|`172.20.11.100/24`|9999|CPU 16|`receiver0`|`UP`，真实三阵面光纤接收口 #1，`MTU = 9000`|
...
| 数据面 MTU (`receiver0`) | NetworkManager `receiver0` 连接 (`802-3-ethernet.mtu`) | `9000` | `ip link show receiver0 && nmcli connection show receiver0 \| grep 802-3-ethernet.mtu` |
...
`receiver0` 巨型帧持久化设置命令：

```bash
sudo nmcli connection modify receiver0 802-3-ethernet.mtu 9000
sudo nmcli connection up receiver0
ip link show receiver0
nmcli connection show receiver0 | grep 802-3-ethernet.mtu
```
...
#### A.2.5 V2.1 网络补充修改 (2026-04-01)

1. 根据现场接收验证需求，将 `receiver0` 的 `MTU` 持久化调整为 `9000`（巨型帧）。
2. 持久化方式采用 NetworkManager 连接 `receiver0` 的 `802-3-ethernet.mtu=9000`。
3. 现场复核结果：
   - `ip link show receiver0` 返回 `mtu 9000`
   - `nmcli connection show receiver0` 返回 `802-3-ethernet.mtu: 9000`
