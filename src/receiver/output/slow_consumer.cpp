...
namespace rxtech
{
    ...
    SlowConsumerHarness::SlowConsumerHarness(SpscRing<CpiOutput> &output_ring,
    ...
    void SlowConsumerHarness::run(const std::atomic<bool> &stop_flag)
    {
        ...
        while (!stop_flag.load(std::memory_order_relaxed))
        {
            ...
            if (output.first_rx_tsc > 0U && exit_tsc > output.first_rx_tsc)
            {
                ...
                const std::uint64_t approx_backlog =
                    (config_.consume_delay_us > 0U && cnt > 0U)
                ...
                {
                    if (stats_.peak_backlog.compare_exchange_weak(
                            prev, approx_backlog,
