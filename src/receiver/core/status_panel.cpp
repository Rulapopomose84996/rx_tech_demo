...
namespace rxtech
{
    ...
    namespace
    {
        ...
        std::string describe_link_state(const RunSummary &summary)
        {
            ...
            {
                return "仅检测到 ARP 探测，尚未收到业务 UDP 流量";
            }
            if (summary.filtered_packets > 0U || summary.raw_rx_packets > 0U)
            {
                return "已检测到非目标流量，当前按过滤规则忽略";
            }
            return "未检测到链路流量";
        }
                                                             ...
                                                             const std::chrono::steady_clock::duration &elapsed)
        {
            ...
            lines.push_back(build_metric_line("CPI 数量", std::to_string(summary.cpi_count)));
            lines.push_back(build_metric_line("PRT 数量", std::to_string(summary.prt_count)));
            lines.push_back(build_metric_line("通道数量", std::to_string(summary.channel_count)));
