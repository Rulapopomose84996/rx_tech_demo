...
std::vector<UdpPayloadFrame> UdpPayloadAssembler::push(const PacketDesc& packet) {
    ...
    if (fragment_offset_bytes == 0U && !more_fragments) {
        ...
        frame.ts_ns = packet.ts_ns;
        frame.port_id = packet.port_id;
        frame.queue_id = packet.queue_id;
        frame.face_id = packet.face_id;
    ...
    if (fragment_offset_bytes == 0U && ip_payload_length >= 8U) {
        ...
        assembly.ts_ns = packet.ts_ns;
        assembly.port_id = packet.port_id;
        assembly.queue_id = packet.queue_id;
        assembly.face_id = packet.face_id;
    }
    ...
    frame.ts_ns = assembly.ts_ns;
    frame.port_id = assembly.port_id;
    frame.queue_id = assembly.queue_id;
    frame.face_id = assembly.face_id;
