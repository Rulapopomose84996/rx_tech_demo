    ...
    InterpretedPacketView ProtocolSequenceInterpreter::interpret(const ParsedPacketView &packet) const noexcept
    {
        ...
        if (!packet.valid)
        {
            result.reject_reason =
                packet.reject_reason == RejectReason::none ? RejectReason::invalid_header : packet.reject_reason;
            return result;
        }
        ...
        result.packet_position_in_prt =
            static_cast<std::uint16_t>(packet.channel * spec_.packets_per_channel + packet.packet_index);
