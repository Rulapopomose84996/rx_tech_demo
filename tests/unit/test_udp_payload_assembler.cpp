...
namespace
{
    ...
    std::vector<std::uint8_t> make_ipv4_udp_frame(const std::vector<std::uint8_t> &udp_payload,
                                                  std::uint16_t identification, bool more_fragments,
                                                  std::uint16_t fragment_offset_bytes,
                                                  std::uint16_t full_udp_length = 0U)
    {
        const std::uint16_t udp_length =
            full_udp_length != 0U ? full_udp_length : static_cast<std::uint16_t>(8U + udp_payload.size());
        const std::uint16_t ip_payload_length = fragment_offset_bytes == 0U
        ...
        std::vector<std::uint8_t> bytes = {
            ...
            0x45,
            0x00,
            static_cast<std::uint8_t>((total_length >> 8U) & 0xFFU),
            static_cast<std::uint8_t>(total_length & 0xFFU),
            static_cast<std::uint8_t>((identification >> 8U) & 0xFFU),
            static_cast<std::uint8_t>(identification & 0xFFU),
            static_cast<std::uint8_t>((fragment_field >> 8U) & 0xFFU),
...
int main()
{
...

    {
        ...
        if (results[0].udp_payload.data() != frame.data() + 42U)
        {
            std::cerr << "expected unfragmented UDP payload path to expose a zero-copy view\n";
            return 1;
        }
    }

    {
        ...
        const std::vector<std::uint8_t> first_fragment =
            make_ipv4_udp_frame(first_payload, 0x1002U, true, 0U, full_udp_length);
        const std::vector<std::uint8_t> second_fragment =
            make_ipv4_udp_frame(second_payload, 0x1002U, false, 1488U, full_udp_length);
    ...
    {
        std::vector<std::uint8_t> udp_payload(2048U, 0xEFU);
        udp_payload[0] = 0x03U;
        udp_payload[1] = 0xFFU;
        udp_payload[2] = 0xAAU;
        udp_payload[3] = 0x55U;
        ...
        const std::vector<std::uint8_t> first_fragment =
            make_ipv4_udp_frame(first_payload, 0x1003U, true, 0U, full_udp_length);
        const std::vector<std::uint8_t> second_fragment =
            make_ipv4_udp_frame(second_payload, 0x1003U, false, 1488U, full_udp_length);

        rxtech::PacketDesc first_packet;
        first_packet.data = const_cast<std::uint8_t *>(first_fragment.data());
        first_packet.len = static_cast<std::uint32_t>(first_fragment.size());
        first_packet.ts_ns = 1U;

        rxtech::PacketDesc stale_second_packet;
        stale_second_packet.data = const_cast<std::uint8_t *>(second_fragment.data());
        stale_second_packet.len = static_cast<std::uint32_t>(second_fragment.size());
        stale_second_packet.ts_ns = kFragmentAssemblyTtlNs + 100U;

        const auto first_results = assembler.push(first_packet);
        if (!first_results.empty())
        {
            std::cerr << "expected first fragment alone to produce no complete UDP payload before ttl test\n";
            return 1;
        }

        const auto stale_results = assembler.push(stale_second_packet);
        if (!stale_results.empty())
        {
            std::cerr << "expected stale fragment assembly to be evicted before the second fragment arrives\n";
            return 1;
        }

        first_packet.ts_ns = kFragmentAssemblyTtlNs + 200U;

        const auto refreshed_results = assembler.push(first_packet);
        if (refreshed_results.size() != 1U || refreshed_results[0].udp_payload.size() != 2048U)
        {
            std::cerr << "expected fresh first fragment to complete the still-live second fragment after stale state "
                         "was evicted\n";
            return 1;
        }
    }
